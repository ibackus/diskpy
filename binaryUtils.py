...
def angMomSearch(name, simUnits=False, flag="AngularMomentum="):
    ...
    if(simUnits == False):
        return res * \
            (AddBinary.Msol * AddBinary.AUCM * 1000 * 100 * AddBinary.VEL_UNIT)
    else:
        return res
...
def changaFloatSearch(name, simUnits=False):
    ...
    # Case: Each element of data is a list of strings
        ...
        for i in range(rows):
            for item in data[i]:
                # Check if list is empty or contains floats.  If not empty,
                # convert to numpy array of floats, store in return array
                tmpList = re.findall(
                    r"[+-]? *(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?",
                    item)
                if tmpList:
    ...
    # Ensure final result is some 2D numpy array so I can access it like
    # final[i,j]
    cols = len(result[0])
...
def linearMomentumEffects(x1, x2, v1, v2, m1, m2, accretion):
    ...
    """
    # Extract masses and velocities of accreted gas particles from array of
    # known format
    ...
    # Loop over accretion events, apply conservation of linear momentum at
    # each step
    for i in range(len(accretion)):
...
def find_crit_radius(r, array, toFind, num=1000):
    ...
    # Estimate surface density as function of radius.  s=0 -> Interpolate
    # through all data points assuming smooth curve
...
def computeCOM(stars, gas, cutoff=None, starFlag=True):
    ...
    """
    # If there's a cutoff, select gas particles with cylindrical radius less
    # than the cutoff
    ...
    if starFlag:  # Include stars
        ...
        return np.sum(
            gas['pos'] * isaac.strip_units(np.mean(gas['mass']))) / np.sum(gas['mass'])
...
def calcDiskRadialBins(s, r_in=0, r_out=0, bins=50):
    ...
    # Ensure you're not right on binary or too far out.  Redundant, but
    # whatever
    mask = (r > r.min()) & (r < r_out)
    r = r[mask]
...
def calcNetTorque(stars, gas):
    ...
    grav = AddBinary.BigG * \
        (stars[0]['mass'] * gas['mass'] / np.power(np.linalg.norm(stars[0]['pos'] - gas['pos'], 1), 3))

    # Scale that value by (x'-x) to make it a vector pointing to gas particles
    F1 = -1 * stars[0]['pos'] + gas['pos']
    conv = (AddBinary.Msol * AddBinary.Msol) / \
        (AddBinary.AUCM * AddBinary.AUCM)
    F1[:, 0] *= (grav * conv)
    F1[:, 1] *= (grav * conv)
    F1[:, 2] *= (grav * conv)
    ...
    grav = AddBinary.BigG * \
        (stars[1]['mass'] * gas['mass'] / np.power(np.linalg.norm(stars[1]['pos'] - gas['pos'], 1), 3))
    ...
    tau1 = np.cross(r1, F1)
    tau2 = np.cross(r2, F2)
    netTau = tau1 / (stars[0]['mass'] * AddBinary.Msol) + \
        tau2 / (stars[1]['mass'] * AddBinary.Msol)
...
def torqueVsRadius(s, rBinEdges):
    ...
    # For a given radius, put gas particles in bins where s.gas['r'] is in au
    # and pynbody units are stripped
    for i in range(0, len(rBinEdges) - 1):
        rMask = np.logical_and(
            isaac.strip_units(
                s.gas['rxy']) > rBinEdges[i], isaac.strip_units(
                s.gas['rxy']) < rBinEdges[
                i + 1])
...
def calcDeDt(stars, tau):
    ...
    v1 = np.asarray(
        isaac.strip_units(
            stars[0]['vel'])) * 1000 * 100 * AddBinary.VEL_UNIT
    v2 = np.asarray(
        isaac.strip_units(
            stars[1]['vel'])) * 1000 * 100 * AddBinary.VEL_UNIT
    ...
    # Calculate change in e vs time due to z component of torque/mass (dh/dt
    # for specific angular momentum h)
...
def estimateCBResonances(s, r_max, m_max=5, l_max=5, bins=2500):
    ...
    for m in range(m_min, m_max):
        for l in range(l_min, l_max):
            # oTmp =
            # find_crit_radius(r,omega_d-(kappa/(float(m))),omega_b,bins)
            # #outer LR
            oTmp = np.power(float(m + 1) / l, 2. / 3.) * a
            omega_Lo[m - m_min, l - l_min] = 2.0 * \
                np.pi / AddBinary.aToP(oTmp, m1 + m2)

        # iTmp = find_crit_radius(r,omega_d+(kappa/(float(m))),omega_b,bins)
        # #inner LR
            iTmp = np.power(float(m - 1) / l, 2. / 3.) * a
            omega_Li[m - m_min, l - l_min] = 2.0 * \
                np.pi / AddBinary.aToP(iTmp, m1 + m2)

    # return inner, outer, co angular frequencies
    return omega_Li, omega_Lo, o_c
...
def findCBResonances(s, r, r_min, r_max, m_max=4, l_max=4, bins=50):
    ...
    # Compute kappa (radial epicycle frequency = sqrt(r * d(omega^2)/dr +
    # 4*(omega^2))
    o2 = omega_d * omega_d
    # Assuming r has evenly spaced bins!
    dr = (r.max() - r.min()) / float(bins)
    ...
    for m in range(m_min, m_max + 1):
        for l in range(l_min, l_max + 1):
            ...
            # Find corotation resonance where omega_d ~ omega_b
            o_c[l -
                l_min] = omega_d[np.argmin(np.fabs(omega_d -
                                                   omega_b /
                                                   float(l)))]
...
def calcCoMVsRadius(s, rBinEdges, starFlag=False):
    ...
    if starFlag:  # Include stars in center of mass calculation
        ...
        for i in range(0, len(rBinEdges) - 1):
            com[i, :] = computeCOM(
                stars, gas, cutoff=rBinEdges[i], starFlag=starFlag)
    else:  # Gas disk only
        for i in range(0, len(rBinEdges) - 1):
            com[i, :] = computeCOM(
                stars, gas, cutoff=rBinEdges[i], starFlag=starFlag)
...
def calcPoissonVsRadius(s, rBinEdges):
    ...
    for i in range(0, len(rBinEdges) - 1):
        rMask = np.logical_and(
            isaac.strip_units(
                gas['rxy']) > rBinEdges[i], isaac.strip_units(
                gas['rxy']) < rBinEdges[
                i + 1])
        N = len(gas[rMask])
...
def calcQVsRadius(s, a_in, a_out, bins):
    ...
    p = pynbody.analysis.profile.Profile(
        s.gas,
        max=a_out,
        min=a_in,
        nbins=bins)
